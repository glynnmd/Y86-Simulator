...
{
   if(low < 0 || low > 63 || high < low || high < 0 || high > 63)
   {
        return 0;
   }
...
{
   if(srclow < 0 || srclow > 63 || dstlow < 0 || dstlow > 63 || length < 1 || length > 64 || (length + srclow) > 64 || (length + dstlow) > 64)
   {
       return dest;
   }
   dest = clearBits(dest, dstlow, dstlow + length - 1);
   source = source << (63 - (srclow + length - 1));
   source = source >> (63 - (srclow + length - 1) + srclow);
 ...
 * and false otherwise
 *
 * for example, subOverflow(0x8000000000000000, 0x8000000000000000) returns 1
 *              subOverflow(0x7fffffffffffffff, 0x7fffffffffffffff) returns 1
 *              subOverflow(0x8000000000000000, 0x7fffffffffffffff) returns 0
 *
